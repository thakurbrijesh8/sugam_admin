...
class Domicile_model extends CI_Model {

    function get_all_domicile_certificate_list($start, $length, $s_district = '', $s_app_no = '', $s_app_date = '', $s_app_det = '', $s_vdw = '', $s_app_status = '', $s_co_hand = '', $s_qstatus = '', $s_status = '') {
        ...
        $this->_bd_for_search_dc($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status);

        $this->db->limit($length, $start);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->where('r.status != ' . VALUE_ZERO);
        $this->db->where('r.status != ' . VALUE_ONE);
        $this->db->from('domicile_certificate AS r');
        ...
        $this->db->join('query AS q', 'q.query_id = (SELECT max(query_id) FROM query WHERE module_type = ' . VALUE_ONE . ' AND module_id = r.domicile_certificate_id AND is_delete != ' . VALUE_ONE . ')', 'LEFT');
    ...
    function get_total_count_of_records($s_district) {
        $this->db->select('COUNT(r.domicile_certificate_id) AS total_records');
        if (!is_admin()) {
    ...
    function _bd_for_search_dc($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status) {
        if ($s_district != '') {
            $this->db->where('r.district', $s_district);
        }
        if ($s_app_no != '') {
        ...
        if ($s_app_det != '') {
            $where = "(r.name_of_applicant LIKE '%$s_app_det%' OR "
        ...
        if ($s_co_hand != '') {
            ...
            if (is_aci_user()) {
                if ($s_co_hand == VALUE_ONE) {
                    $this->db->where('((r.aci_to_mamlatdar=' . VALUE_ZERO . ' AND r.aci_to_ldc=' . VALUE_ZERO . ') AND r.to_type_reverify=' . VALUE_ZERO . ')');
                }
                if ($s_co_hand == VALUE_TWO) {
                ...
                if ($s_co_hand == VALUE_FOUR) {
                    $this->db->where('((r.aci_rec_reverify=' . VALUE_ONE . ' OR r.aci_rec_reverify=' . VALUE_TWO . ') '
            ...
            if (is_ldc_user()) {
                if ($s_co_hand == VALUE_ONE) {
                    $this->db->where('(r.aci_rec=' . VALUE_ONE . ' AND r.ldc_to_mamlatdar=' . VALUE_ZERO . ' AND r.to_type_reverify=' . VALUE_ZERO . ')');
                }
                if ($s_co_hand == VALUE_TWO) {
                ...
                if ($s_co_hand == VALUE_THREE) {
                    $this->db->where('((r.aci_to_ldc!=' . VALUE_ZERO . ' AND r.aci_rec_reverify=' . VALUE_ONE . ') '
                            . 'AND (r.to_type_reverify=' . VALUE_ONE . ' OR r.to_type_reverify=' . VALUE_TWO . ') '
            ...
            if (is_mamlatdar_user()) {
                ...
                if ($s_co_hand == VALUE_THREE) {
                    $this->db->where('((r.to_type_reverify=' . VALUE_ONE . ' OR r.to_type_reverify=' . VALUE_TWO . ') AND '
                            ...
                            . '(r.aci_rec_reverify=' . VALUE_ONE . ' AND r.ldc_to_mamlatdar!=' . VALUE_ZERO . ')))');
                }
                if ($s_co_hand == VALUE_FOUR) {
                    $this->db->where('((r.to_type_reverify=' . VALUE_ONE . ' OR r.to_type_reverify=' . VALUE_TWO . ') AND '
    ...
    function get_records_for_excel($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status) {
        $this->db->select('r.application_number, r.submitted_datetime, r.name_of_applicant, r.mobile_number, '
        ...
        $this->db->from('domicile_certificate AS r');
        $this->db->join('users as u', 'u.user_id = r.user_id', 'left');
        $this->db->order_by('r.domicile_certificate_id', 'DESC');
        $resc = $this->db->get();
        return $resc->result_array();
    }

    function get_basic_data_for_dc($domicile_certificate_id) {
        ...
        $this->db->where('r.domicile_certificate_id', $domicile_certificate_id);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->where('r.status != ' . VALUE_ZERO);
        $this->db->where('r.status != ' . VALUE_ONE);
        $this->db->from('domicile_certificate AS r');
    ...
    function get_name_result_data_for_dc($field_name, $field_value, $table_name, $order_by_id = NULL, $order_by = NULL) {
