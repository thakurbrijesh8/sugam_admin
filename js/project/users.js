...
Users.listView = Backbone.View.extend({
    ...
    askForNewUsersForm: function (btnObj) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                tempRuralVillagesData = parseData.rural_villages_data;
    ...
    newUsersForm: function (isEdit, usersData) {
        ...
        if (isEdit) {
            ...
            var tempData = tempRuralVillagesData[usersData.district] ? tempRuralVillagesData[usersData.district] : [];
            if ((usersData.user_type == VALUE_TWO && usersData.district == TALUKA_DAMAN) || (usersData.user_type == VALUE_TWO && usersData.district == TALUKA_DIU)) {
                $('#villages_container_for_users').show();
                generateCheckBoxesForVillages('checkbox', tempData, 'assign_villages', 'users', false, false);
                if (usersData.district == TALUKA_DAMAN) {
                    generateBoxes('checkbox', damanNCVillagesArray, 'assign_villages', 'users', usersData.assign_villages, false);
                } else if (usersData.district == TALUKA_DIU) {
                    generateBoxes('checkbox', diuNCVillagesArray, 'assign_villages', 'users', usersData.assign_villages, false);
                }
            }
        }
        generateSelect2();
    ...
    villageShowHideChangeEvent: function () {
        var userType = $('#user_type_for_users').val();
        var dist = $('#district_for_users').val();
        $('#villages_container_for_users').hide();

        if (userType != VALUE_TWO || (dist != TALUKA_DAMAN && dist != TALUKA_DIU && dist != TALUKA_DNH)) {
            return false;
        }
        var tempData = tempRuralVillagesData[dist] ? tempRuralVillagesData[dist] : [];
        if (dist == TALUKA_DAMAN || dist == TALUKA_DIU) {
            $('#villages_container_for_users').show();
            generateCheckBoxesForVillages('checkbox', tempData, 'assign_villages', 'users', tempData.village_name, false);
            if (dist == TALUKA_DAMAN) {
                generateBoxes('checkbox', damanNCVillagesArray, 'assign_villages', 'users', tempData.village_name, false);
            } else if (dist == TALUKA_DIU) {
                generateBoxes('checkbox', diuNCVillagesArray, 'assign_villages', 'users', tempData.village_name, false);
            }
        }
    }
