...
class Death_certificate_model extends CI_Model {
    ...
    function get_all_death_certificate_list($start, $length, $s_district = '', $s_app_no = '', $s_app_det = '', $s_app_status = '', $s_qstatus = '', $s_status = '') {
        ...
        $this->_bd_for_search_dc($s_district, $s_app_no, $s_app_det, $s_app_status, $s_qstatus, $s_status);

        $this->db->limit($length, $start);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->where('r.status != ' . VALUE_ZERO);
        $this->db->where('r.status != ' . VALUE_ONE);
        $this->db->from('death_certificate AS r');
        $this->db->join('sa_users as sau', 'sau.sa_user_id = r.updated_by', 'left');
        $this->db->join('query AS q', 'q.query_id = (SELECT max(query_id) FROM query WHERE module_type = ' . VALUE_TWENTY . ' AND module_id = r.death_certificate_id AND is_delete != ' . VALUE_ONE . ')', 'LEFT');
    ...
    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(r.death_certificate_id) AS total_records');
        if (!is_admin()) {
    ...
    function _bd_for_search_dc($s_district, $s_app_no, $s_app_det, $s_app_status, $s_qstatus, $s_status) {
        if ($s_district != '') {
    ...
    function get_records_for_excel($s_district, $s_app_no, $s_app_det, $s_app_status, $s_qstatus, $s_status) {
        $this->db->select('r.application_number, r.submitted_datetime, r.applicant_name, r.communication_address, r.mobile_number, '
                . 'r.district,r.death_person_dod,r.death_place,r.registration_number, r.status, r.query_status');
        ...
        $this->db->from('death_certificate AS r');
        $this->db->join('users as u', 'u.user_id = r.user_id', 'left');
        $this->db->order_by('r.death_certificate_id', 'DESC');
        $resc = $this->db->get();
        return $resc->result_array();
    }

    function get_basic_data_for_dc($death_certificate_id) {
        $this->db->select('r.*, sau.name AS actioner_user_name');
        $this->db->where('r.death_certificate_id', $death_certificate_id);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->where('r.status != ' . VALUE_ZERO);
        $this->db->where('r.status != ' . VALUE_ONE);
        $this->db->from('death_certificate AS r');
...
/*
 * EOF: ./application/models/Death_Certificate_model.php
 */