...
class Heirship_model extends CI_Model {

    function get_all_heirship_list($start, $length, $s_district = '', $s_app_no = '', $s_app_date = '', $s_app_det = '', $s_vdw = '', $s_app_status = '', $s_co_hand = '', $s_qstatus = '', $s_status = '') {
        ...
        $this->_bd_for_search_hc($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status);

        $this->db->limit($length, $start);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->where('r.status != ' . VALUE_ZERO);
        $this->db->where('r.status != ' . VALUE_ONE);
        $this->db->from('heirship AS r');
        ...
        $this->db->join('query AS q', 'q.query_id = (SELECT max(query_id) FROM query WHERE module_type = ' . VALUE_THREE . ' AND module_id = r.heirship_id AND is_delete != ' . VALUE_ONE . ')', 'LEFT');
    ...
    function get_total_count_of_records($s_district) {
        $this->db->select('COUNT(r.heirship_id) AS total_records');
        if (!is_admin()) {
    ...
    function _bd_for_search_hc($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status) {
        if ($s_district != '') {
            $this->db->where('r.district', $s_district);
        }
        if ($s_app_no != '') {
        ...
        if ($s_app_det != '') {
            $where = "(r.applicant_name LIKE '%$s_app_det%' OR "
                    . "r.pre_house_no LIKE '%$s_app_det%' OR "
                    . "r.pre_house_name LIKE '%$s_app_det%' OR "
                    . "r.pre_street LIKE '%$s_app_det%' OR "
                    . "r.pre_village LIKE '%$s_app_det%' OR "
                    . "r.pre_city LIKE '%$s_app_det%' OR "
                    . "r.pre_pincode LIKE '%$s_app_det%' OR "
    ...
    function get_records_for_excel($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status) {
        ...
        $this->db->from('heirship AS r');
        $this->db->join('users as u', 'u.user_id = r.user_id', 'left');
        $this->db->order_by('r.heirship_id', 'DESC');
        $resc = $this->db->get();
        return $resc->result_array();
    }

    function get_basic_data_for_hc($heirship_id) {
        ...
        $this->db->where('r.heirship_id', $heirship_id);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->where('r.status != ' . VALUE_ZERO);
        $this->db->where('r.status != ' . VALUE_ONE);
        $this->db->from('heirship AS r');
