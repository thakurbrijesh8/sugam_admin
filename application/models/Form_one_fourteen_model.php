...
class Form_one_fourteen_model extends CI_Model {

    function get_all_form_one_fourteen_list($start, $length, $search_district = '', $search_application_number = '', $search_application_date = '', $search_applicant_details = '', $search_vdw = '', $s_qstatus = '', $search_status = '') {
        ...
        $this->_bd_for_search_fof($search_district, $search_application_number, $search_application_date, $search_applicant_details, $search_vdw, $s_qstatus, $search_status);

        $this->db->limit($length, $start);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->where('r.status != ' . VALUE_ZERO);
        $this->db->where('r.status != ' . VALUE_ONE);
        $this->db->from('form_one_fourteen AS r');
        $this->db->join('sa_users as sau', 'sau.sa_user_id = r.updated_by', 'left');
        $this->db->join('query AS q', 'q.query_id = (SELECT max(query_id) FROM query WHERE module_type = ' . VALUE_THIRTEEN . ' AND module_id = r.form_one_fourteen_id AND is_delete != ' . VALUE_ONE . ')', 'LEFT');
    ...
    function _bd_for_search_fof($search_district, $search_application_number, $search_application_date, $search_applicant_details, $search_vdw, $s_qstatus, $search_status) {
        if ($search_district != '') {
    ...
    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(r.form_one_fourteen_id) AS total_records');
        if ($search_district != '') {
            $this->db->where('r.district', $search_district);
        }
    ...
    function get_form_data_with_land_details($module_type, $form_land_details_id, $is_full_data = false) {
        $sql = 'fof.form_one_fourteen_id, fof.district, fof.application_number, fof.status, fof.land_details, '
                . 'fof.applicant_name, fof.father_name, fof.surname, fof.spouse_name, fof.mobile_number, fof.email, '
                . 'fof.address, fof.purpose, fof.total_copies, fof.total_amount, fof.village, fld.*';
        if ($is_full_data != false) {
            $sql .= ', drv.village_name, pd.reference_id, pd.op_transaction_datetime';
        }
        $this->db->select($sql);
        ...
        $this->db->join('form_one_fourteen AS fof', 'fld.module_id=fof.form_one_fourteen_id');
        if ($is_full_data != false) {
