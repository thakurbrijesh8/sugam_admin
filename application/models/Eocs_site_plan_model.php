...
class Eocs_site_plan_model extends CI_Model {

    function get_all_eocs_site_plan_list($start, $length, $search_district = '', $search_application_number = '', $search_application_date = '', $search_applicant_details = '', $search_area_type = '', $search_village = '', $s_qstatus = '', $search_status = '', $s_plan_status = '') {
        $this->db->select('r.*, sau.name AS actioner_user_name,q.query_id, q.query_by_name, q.query_datetime');

        $this->_bd_for_search_esp($search_district, $search_application_number, $search_application_date, $search_applicant_details, $search_area_type, $search_village, $s_qstatus, $search_status, $s_plan_status);

        $this->db->limit($length, $start);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->from('eocs_site_plan AS r');
        $this->db->join('query AS q', 'q.query_id = (SELECT max(query_id) FROM query WHERE module_type = ' . VALUE_TWENTYTHREE . ' AND module_id = r.eocs_site_plan_id AND is_delete != ' . VALUE_ONE . ')', 'LEFT');
    ...
    function _bd_for_search_esp($search_district, $search_application_number, $search_application_date, $search_applicant_details, $search_area_type, $search_village, $s_qstatus, $search_status, $s_plan_status) {
        if ($search_district != '') {
    ...
    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(r.eocs_site_plan_id) AS total_records');
        if ($search_district != '') {
            $this->db->where('r.district', $search_district);
        }
    ...
    function get_filter_count_of_records($search_district = '', $search_application_number = '', $search_application_date = '', $search_applicant_details = '', $search_area_type = '', $search_village = '', $s_qstatus = '', $search_status = '', $s_plan_status = '') {
        $this->db->select('COUNT(r.eocs_site_plan_id) AS total_records');
...
//    }
    ...
    function get_esp_data_with_land_details($module_type, $form_land_details_id, $is_full_data = false) {
        $sql = 'esp.eocs_site_plan_id, esp.district, esp.application_number, esp.status, esp.plan_status, esp.land_details, '
                . 'esp.applicant_name, esp.father_name, esp.surname, esp.spouse_name, esp.mobile_number, esp.email, '
                . 'esp.address, esp.purpose, esp.total_copies, esp.total_amount, esp.ld_area_type, esp.ld_village_sc, '
                . 'fld.*';
        ...
        $this->db->join('eocs_site_plan AS esp', 'fld.module_id=esp.eocs_site_plan_id');
        if ($is_full_data != false) {
            $this->db->join('fees_payment AS pd', 'pd.reference_number=esp.last_op_reference_number');
        }
        $resc = $this->db->get();
...
/*
 * EOF: ./application/models/Eocs_site_plan_model.php
 */