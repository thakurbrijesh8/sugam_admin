...
class Payment_model extends CI_Model {
    ...
    function get_all_payment_history($start, $length, $search_district = '', $assing_modules = array(), $search_ps = '', $search_prn = '', $search_an = '') {
        $this->db->select('fees_payment_id, op_order_number, reference_id, district, module_type, module_id, application_number, total_fees, op_status, '
                . 'op_start_datetime, reference_number, op_transaction_datetime, op_bank_code, op_bank_reference_number, op_message');
        if ($search_district != '') {
        ...
        if ($search_an != '') {
            $this->db->like('application_number', $search_an);
        }
    ...
    function get_filter_count_of_records($search_district = '', $assing_modules = array(), $search_ps = '', $search_prn = '', $search_an = '') {
        $this->db->select('COUNT(fees_payment_id) AS total_records');
        if ($search_district != '') {
            $this->db->where('district', $search_district);
        }
    ...
    function get_md_hwr_details($module_type = '', $search_district = '', $full_head = '', $from_date = '', $to_date = '') {
        $this->db->select('fb.module_type, fb.module_id, fb.fee_description, fb.fee, fp.total_fees, fp.application_number, '
        ...
        if ($full_head != '') {
            $this->db->where("(dfd.full_head='$full_head')");
        }
        if ($from_date != '' && $to_date == '') {
            $this->db->where("DATE_FORMAT(fp.op_transaction_datetime,'%Y-%m-%d')", $from_date);
        }
    ...
    function get_username_for_pg($user_id) {
        $this->db->select('lld.sa_logs_login_details_id, u.sa_user_id, u.username,u.user_type,u.district,u.assign_villages');
        $this->db->where('lld.sa_user_id', $user_id);
        $this->db->where('lld.logout_timestamp', 0);
        $this->db->where('u.is_deactive', IS_ACTIVE);
    ...
    function get_pending_dv_data($session_user_id) {
        ...
        $this->db->where('sa_user_id', $session_user_id);
        $this->db->from('fees_payment');
        $resc = $this->db->get();
        return $resc->result_array();
    }
