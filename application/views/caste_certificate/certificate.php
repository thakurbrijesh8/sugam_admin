...
$father_details = $caste_certificate_data['father_details'] ? json_decode($caste_certificate_data['father_details'], true) : array();
...
<hr>
<?php
$commu_address = $caste_certificate_data['ldc_commu_address'] != '' ? $caste_certificate_data['ldc_commu_address'] :
        (($caste_certificate_data['com_addr_house_no'] != '' ? ($caste_certificate_data['com_addr_house_no'] . ', ') : '')
        . ($caste_certificate_data['com_addr_house_name'] != '' ? ($caste_certificate_data['com_addr_house_name'] . ', ') : '')
        . ($caste_certificate_data['com_addr_street'] != '' ? ($caste_certificate_data['com_addr_street'] . ', ') : '')
        . ($caste_certificate_data['com_addr_village_dmc_ward'] != '' ? ($caste_certificate_data['com_addr_village_dmc_ward'] . ', ') : '')
        . ($caste_certificate_data['com_addr_city'] != '' ? ($caste_certificate_data['com_addr_city']) : ''));
...
<table class="f-s-14px read">
    ...
    <tr>
        ...
        <td class="f-w-b">
            <?php
            if ($caste_certificate_data['aci_rec'] == VALUE_ONE || $caste_certificate_data['aci_rec'] == VALUE_THREE) {
...
<div class="f-s-14px" style="margin-left: 50px; margin-bottom: 10px;word-spacing: 8px;margin-top: 20px; line-height: 25px;
    ...
    and <b style="text-decoration: underline; text-transform: capitalize;">R/o. <?php echo $commu_address; ?></b>
    in District <?php echo $taluka_name; ?> of the Union Territory of Dadra & Nagar Haveli and Daman & Diu belongs to the Tribe
...
<?php
$constitution_artical = intval($caste_certificate_data['constitution_artical']);
...
if ($constitution_artical == VALUE_TWO && $caste_certificate_data['minor_child_photo_doc'] != '') {
    $sign_data['applicant_photo_doc_path'] = CASTE_CERTIFICATE_DOC_PATH . $caste_certificate_data['minor_child_photo_doc'];
} else {
    $sign_data['applicant_photo_doc_path'] = CASTE_CERTIFICATE_DOC_PATH . $caste_certificate_data['applicant_photo_doc'];
}
