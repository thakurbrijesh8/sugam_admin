...
class Users extends CI_Controller {
    ...
    function get_user_data_by_id() {
        try {
            ...
            $success_array['rural_villages_data'][TALUKA_DAMAN] = $this->utility_model->get_villages_data('daman_rural_villages');
            $success_array['rural_villages_data'][TALUKA_DIU] = $this->utility_model->get_villages_data('diu_rural_villages');
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
    ...
    function save_user() {
        try {
            ...
            $this->_get_assign_villages($user_data);
    ...
    function _get_assign_villages(&$user_data) {
        $assign_villages = $this->input->post('assign_villages_for_users');
        if ($assign_villages) {
            $daman_cv_array = $this->config->item('daman_cv_array');
            $diu_cv_array = $this->config->item('diu_cv_array');
            if (is_array($assign_villages)) {
                foreach ($assign_villages as $villages) {
                    if ($user_data['district'] == VALUE_ONE) {
                        if (isset($daman_cv_array[$villages])) {
                            array_push($assign_villages, $daman_cv_array[$villages]);
                        }
                    } elseif ($user_data['district'] == VALUE_TWO) {
                        if (isset($diu_cv_array[$villages])) {
                            array_push($assign_villages, $diu_cv_array[$villages]);
                        }
                    }
                }
                $user_data['assign_villages'] = implode(',', $assign_villages);
            } else {
                if ($user_data['district'] == VALUE_ONE) {
                    $user_data['assign_villages'] = $assign_villages . (isset($daman_cv_array[$assign_villages]) ? (',' . $daman_cv_array[$assign_villages]) : '');
                } elseif ($user_data['district'] == VALUE_TWO) {
                    $user_data['assign_villages'] = $assign_villages . (isset($diu_cv_array[$assign_villages]) ? (',' . $diu_cv_array[$assign_villages]) : '');
                }
            }
        } else {
            $user_data['assign_villages'] = '';
        }
    }
    ...
    function get_common_data_for_user() {
        try {
            ...
            $success_array = get_success_array();
            $success_array['user_type_data'] = array();
            $success_array['rural_villages_data'] = array();
            if ($user_id == NULL || !$user_id) {
                echo json_encode($success_array);
                return false;
            }
