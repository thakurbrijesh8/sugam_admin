...
class Form_three_model extends CI_Model {

    function get_all_form_three_list($start, $length, $search_district = '', $search_application_number = '', $search_application_date = '', $search_applicant_details = '', $search_vdw = '', $s_qstatus = '', $search_status = '') {
        ...
        $this->_bd_for_search_ft($search_district, $search_application_number, $search_application_date, $search_applicant_details, $search_vdw, $s_qstatus, $search_status);

        $this->db->limit($length, $start);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->where('r.status != ' . VALUE_ZERO);
        $this->db->where('r.status != ' . VALUE_ONE);
        $this->db->from('form_three AS r');
        $this->db->join('sa_users as sau', 'sau.sa_user_id = r.updated_by', 'left');
        $this->db->join('query AS q', 'q.query_id = (SELECT max(query_id) FROM query WHERE module_type = ' . VALUE_FOURTEEN . ' AND module_id = r.form_three_id AND is_delete != ' . VALUE_ONE . ')', 'LEFT');
    ...
    function _bd_for_search_ft($search_district, $search_application_number, $search_application_date, $search_applicant_details, $search_vdw, $s_qstatus, $search_status) {
        if ($search_district != '') {
    ...
    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(r.form_three_id) AS total_records');
        if ($search_district != '') {
            $this->db->where('r.district', $search_district);
        }
