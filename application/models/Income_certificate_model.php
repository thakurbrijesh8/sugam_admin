...
class Income_certificate_model extends CI_Model {

    function get_all_income_certificate_list($start, $length, $s_district = '', $s_app_no = '', $s_app_date = '', $s_app_det = '', $s_vdw = '', $s_app_status = '', $s_co_hand = '', $s_qstatus = '', $s_status = '') {
        ...
        $this->_bd_for_search_ic($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status);

        $this->db->limit($length, $start);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->where('r.status != ' . VALUE_ZERO);
        $this->db->where('r.status != ' . VALUE_ONE);
        $this->db->from('income_certificate AS r');
        ...
        $this->db->join('query AS q', 'q.query_id = (SELECT max(query_id) FROM query WHERE module_type = ' . VALUE_TWO . ' AND module_id = r.income_certificate_id AND is_delete != ' . VALUE_ONE . ')', 'LEFT');
    ...
    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(r.income_certificate_id) AS total_records');
        if (!is_admin()) {
    ...
    function _bd_for_search_ic($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status) {
        if ($s_district != '') {
        ...
        if ($s_vdw != '') {
            $this->db->where('r.village_dmc_ward', $s_vdw);
        }
    ...
    function _bd_for_session_id() {
        ...
        if (is_talathi_user()) {
            ...
            if ($av != '') {
                $this->db->where_in('village_dmc_ward', explode(',', $av));
            }
        }
        if (is_aci_user()) {
    ...
    function get_records_for_excel($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status) {
        $this->db->select('r.application_number, r.submitted_datetime, r.applicant_name, r.communication_address, r.mobile_number, '
                . 'r.district, r.village_dmc_ward, IF(r.income_by_talathi_reverify!=0, r.income_by_talathi_reverify, r.income_by_talathi) AS income, r.status, r.query_status');
        ...
        $this->db->from('income_certificate AS r');
        $this->db->join('users as u', 'u.user_id = r.user_id', 'left');
        $this->db->order_by('r.income_certificate_id', 'DESC');
        $resc = $this->db->get();
        return $resc->result_array();
    }

    function get_basic_data_for_ic($income_certificate_id) {
        ...
        $this->db->where('r.income_certificate_id', $income_certificate_id);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->where('r.status != ' . VALUE_ZERO);
        $this->db->where('r.status != ' . VALUE_ONE);
        $this->db->from('income_certificate AS r');
