...
class Ews_certificate_model extends CI_Model {

    function get_all_ews_certificate_list($start, $length, $s_district = '', $s_app_no = '', $s_app_date = '', $s_app_det = '', $s_vdw = '', $s_app_status = '', $s_co_hand = '', $s_qstatus = '', $s_status = '') {
        ...
        $this->_bd_for_search_ec($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status);

        $this->db->limit($length, $start);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->where('r.status != ' . VALUE_ZERO);
        $this->db->where('r.status != ' . VALUE_ONE);
        $this->db->from('ews_certificate AS r');
        ...
        $this->db->join('query AS q', 'q.query_id = (SELECT max(query_id) FROM query WHERE module_type = ' . VALUE_SEVEN . ' AND module_id = r.ews_certificate_id AND is_delete != ' . VALUE_ONE . ')', 'LEFT');
    ...
    function get_total_count_of_records($search_district) {
        $this->db->select('COUNT(r.ews_certificate_id) AS total_records');
        if (!is_admin()) {
    ...
    function _bd_for_search_ec($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status) {
        if ($s_district != '') {
    ...
    function get_records_for_excel($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status) {
        ...
        $this->db->from('ews_certificate AS r');
        $this->db->join('users as u', 'u.user_id = r.user_id', 'left');
        $this->db->order_by('r.ews_certificate_id', 'DESC');
        $resc = $this->db->get();
        return $resc->result_array();
    }

    function get_basic_data_for_ec($ews_certificate_id) {
        ...
        $this->db->where('r.ews_certificate_id', $ews_certificate_id);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->where('r.status != ' . VALUE_ZERO);
        $this->db->where('r.status != ' . VALUE_ONE);
        $this->db->from('ews_certificate AS r');
    ...
    function get_name_result_data_for_ec($field_name, $field_value, $table_name, $order_by_id = NULL, $order_by = NULL) {
...
/*
 * EOF: ./application/models/EWS_model.php
 */