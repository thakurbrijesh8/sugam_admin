...
class Caste_certificate_model extends CI_Model {

    function get_all_caste_certificate_list($start, $length, $s_district = '', $s_app_no = '', $s_app_date = '', $s_app_det = '', $s_vdw = '', $s_app_status = '', $s_co_hand = '', $s_qstatus = '', $s_status = '') {
        ...
        $this->_bd_for_search_cc($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status);

        $this->db->limit($length, $start);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->where('r.status != ' . VALUE_ZERO);
        $this->db->where('r.status != ' . VALUE_ONE);
        $this->db->from('caste_certificate AS r');
        ...
        $this->db->join('query AS q', 'q.query_id = (SELECT max(query_id) FROM query WHERE module_type = ' . VALUE_FOUR . ' AND module_id = r.caste_certificate_id AND is_delete != ' . VALUE_ONE . ')', 'LEFT');
    ...
    function get_total_count_of_records($s_district) {
        $this->db->select('COUNT(r.caste_certificate_id) AS total_records');
        if (!is_admin()) {
    ...
    function _bd_for_search_cc($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status) {
        if ($s_district != '') {
            $this->db->where('r.district', $s_district);
        }
        if ($s_app_no != '') {
        ...
        if ($s_app_det != '') {
            $where = "(r.applicant_name LIKE '%$s_app_det%' OR "
                    ...
                    . "r.com_addr_pincode  LIKE '%$s_app_det%' OR "
    ...
    function get_records_for_excel($s_district, $s_app_no, $s_app_date, $s_app_det, $s_vdw, $s_app_status, $s_co_hand, $s_qstatus, $s_status) {
        ...
        $this->db->from('caste_certificate AS r');
        $this->db->join('users as u', 'u.user_id = r.user_id', 'left');
        $this->db->order_by('r.caste_certificate_id', 'DESC');
        $resc = $this->db->get();
        return $resc->result_array();
    }
    ...
    function get_basic_data_for_cc($caste_certificate_id) {
        ...
        $this->db->where('r.caste_certificate_id', $caste_certificate_id);
        $this->db->where('r.is_delete !=' . IS_DELETE);
        $this->db->where('r.status != ' . VALUE_ZERO);
        $this->db->where('r.status != ' . VALUE_ONE);
        $this->db->from('caste_certificate AS r');
