...
class Payment_status extends CI_Controller {
    ...
    function payment_failed() {
        ...
        if (count($temp_od) == VALUE_THREE && isset($temp_od[2])) {
            //For Financial Year Inculded in Application Number Ex.2023-24, 2024-25, 2025-26
            $temp_od[1] .= '-' . $temp_od[2];
            unset($temp_od[2]);
        }
        ...
        $ex_udata = $this->payment_model->get_username_for_pg($module_data['sa_user_id']);
        ...
        if ($fp['op_status'] == VALUE_THREE) {
            ...
            $redirect_data['pg_class'] = 'color-nic-red';
            $redirect_data['pg_icon'] = 'times-circle';
        ...
        if (!empty($ex_udata)) {
            $session_data = array();
            $session_data['temp_id_for_sugam_admin'] = $ex_udata['sa_user_id'];
            $session_data['name'] = ucwords($ex_udata['username']);
            $session_data['temp_type_for_sugam_admin'] = $ex_udata['user_type'];
            $session_data['temp_district_for_sugam_admin'] = $ex_udata['district'];
            $session_data['temp_av_for_sugam_admin'] = $ex_udata['assign_villages'];
            $session_data['temp_logged'] = encrypt($ex_udata['sa_logs_login_details_id']);
    ...
    function payment_success() {
        ...
        $redirect_data['pg_icon'] = 'check-circle';
