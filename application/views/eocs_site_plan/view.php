<div class="card-header">
    <h3 class="card-title f-w-b" style="float: none; text-align: center;">
        {{title}} Application for issue of Site Plan (Urban Area)
    </h3>
</div>
<div class="card-body p-b-0px text-left" style="font-size: 13px;">
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-padding">
            ...
            <tbody id="fld_container_for_espview"></tbody>
            <tfoot>
                <tr>
                    <td class="f-w-b text-right text-success v-a-m bg-beige" colspan="6">Rupees To Be Paid : </td>
                    <th class="f-w-b text-right text-success v-a-m bg-beige">Copies : <span id="total_copies_for_espview"></span></th>
                    <th class="f-w-b text-right text-success v-a-m bg-beige">Rs. : <span id="total_amount_for_espview"></span></th>
                </tr>
            </tfoot>
        </table>
    </div>
            ...
            <tr>
                ...
                <td style="width: 180px; border-top: none;">
                    <img style="border: 2px solid blue; width: 160px; height: 180px;"
                         src="{{EOCS_SITE_PLAN_DOC_PATH}}{{applicant_photo}}">
                </td>
                {{/if}}
                <td style="border-top: none; vertical-align: top !important;">
                    <div>
                        {{#if show_nakal}}
                        <a target="_blank" href="{{EOCS_SITE_PLAN_DOC_PATH}}{{nakal}}">
                            <label class="btn btn-sm btn-nic-blue f-w-n cursor-pointer mb-1">
                                <i class="fas fa-download"></i> &nbsp; Property Card / Form B
                            </label>
                        </a>
                        {{/if}}
                        {{#if show_id_proof}}
                        <a target="_blank" href="{{EOCS_SITE_PLAN_DOC_PATH}}{{id_proof}}">
                            <label class="btn btn-sm btn-nic-blue f-w-n cursor-pointer mb-1">
    ...
    <hr>
    <input type="hidden" id="eocs_site_plan_id_for_espview" name="eocs_site_plan_id_for_espview" value="{{eocs_site_plan_id}}" />
    <div class="row">
        <div class="form-group col-sm-6">
            <label>Remarks <span class="color-nic-red">*</span></label>
            <textarea id="remarks_for_espview" name="remarks_for_espview"
                      onblur="checkValidation('espview', 'remarks_for_espview', remarksValidationMessage);"
                      class="form-control" placeholder="Enter Remarks !" maxlength="200" ></textarea>
            <span class="error-message error-message-espview-remarks_for_espview"></span>
        </div>
    </div>
    {{/if}}
</div>
<div class="card-footer text-right">
    {{#if show_print_btn}}
    <button type="button" class="btn btn-sm btn-danger" onclick="window.print();" id="pa_btn_for_espview">
        <i class="fas fa-file-pdf mr-1"></i> Print Application
    </button>
    {{/if}}
    {{#if show_verification_btn}}
    <button type="button" class="btn btn-sm btn-success"
            onclick="EocsSitePlan.listview.updateApplicationStatus($(this), VALUE_ONE);">
        <i class="fas fa-clipboard-check mr-1"></i> Verify & Send for Payment
    </button>
    {{/if}}
    {{#if show_prepare_btn}}
    <button type="button" class="btn btn-sm btn-success"
            onclick="EocsSitePlan.listview.updateApplicationStatus($(this), VALUE_TWO);">
        <i class="fas fa-clipboard-check mr-1"></i> Confirm Prepared By
    </button>
    {{/if}}
    {{#if show_check_btn}}
    <button type="button" class="btn btn-sm btn-success"
            onclick="EocsSitePlan.listview.updateApplicationStatus($(this), VALUE_THREE);">
