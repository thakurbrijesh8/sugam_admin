...
class Vp_users extends CI_Controller {
    ...
    function get_vp_users_data() {
        try {
            ...
            $this->db->trans_start();
            $vp_user_data = $this->vp_users_model->get_all_vp_users_list($start, $length, $search_applicant_name, $search_applicant_mobile, $search_applicant_email);
    ...
    function resend_verification_link() {
        try {
            ...
            $link = PROJECT_PATH . 'confirmation?q=' . $tu_data['temp_access_token'];
            $registration_message = "Click following URL or Paste URL in browser's address bar to complete your Account Verification for <b>Sugam</b> <br><br>" . $link;
            $this->load->helper('sms_helper');
            send_SMS($this, $tu_data['user_id'], $tu_data['mobile_number'], VALUE_ONE, VALUE_ZERO, VALUE_ZERO, VALUE_ZERO, $tu_data['temp_access_token']);
